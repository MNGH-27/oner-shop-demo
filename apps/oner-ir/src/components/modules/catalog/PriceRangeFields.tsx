...
export function PriceRangeFields({ minPrice, maxPrice, rangeMin, rangeMax }: { minPrice?: string; maxPrice?: string; rangeMin: number; rangeMax: number }) {
  ...
  return <><PriceField key={`min-${initialMinimum}-${minimum}-${maximum}`} name="minPrice" label="حداقل قیمت" initial={initialMinimum} minimum={minimum} maximum={maximum}/><PriceField key={`max-${initialMaximum}-${minimum}-${maximum}`} name="maxPrice" label="حداکثر قیمت" initial={initialMaximum} minimum={minimum} maximum={maximum}/></>;
}
