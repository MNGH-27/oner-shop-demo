...
export function ProductPurchase({ product }: { product: StoreProduct }) {
  ...
  return <div className="purchase-box">
    {product.colors?.length ? <fieldset><legend>رنگ</legend><div className="choice-row color-choices">{product.colors.map((item) => { const exists = !product.variants?.length || product.variants.some((entry) => entry.color === item.name); return <button type="button" key={item.name} disabled={!exists} className={color === item.name ? "selected" : ""} onClick={() => chooseColor(item.name)} title={item.hex ? `${item.name} — ${item.hex}` : item.name}><span className={`color-swatch ${item.hex ? "" : "color-swatch-empty"}`} style={{ backgroundColor: item.hex ?? "#f3efe7" }} aria-hidden="true" />{item.name}</button> })}</div></fieldset> : null}
