...
export function ProductPurchase({ product }: { product: StoreProduct }) {
  ...
  return <div className="purchase-box">
    {product.colors?.length ? <fieldset><legend>رنگ</legend><div className="choice-row color-choices">{product.colors.map((item) => { const exists = !product.variants?.length || product.variants.some((entry) => entry.color === item.name); return <button type="button" key={item.name} disabled={!exists} className={color === item.name ? "selected" : ""} onClick={() => chooseColor(item.name)}><span className="color-swatch" style={{ backgroundColor: item.hex ?? "transparent" }} aria-hidden="true" />{item.name}</button> })}</div></fieldset> : null}
