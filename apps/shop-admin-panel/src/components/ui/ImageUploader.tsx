...
export function ImageUploader({
  ...
  async function handleChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (invalidType) {
      setError('فقط تصاویر JPG، PNG، WebP و GIF قابل آپلود هستند.')
      return
    }
    const oversized = files.find((file) => file.size > 5 * 1024 * 1024)
    if (oversized) {
      setError(`حجم تصویر «${oversized.name}» نباید بیشتر از ۵ مگابایت باشد.`)
      return
    }
    if (multiple && value.length + files.length > 10) {
      setError('حداکثر ۱۰ تصویر برای هر محصول قابل انتخاب است.')
      return
    }
