...
@ApiTags('Admin - Banners') @ApiBearerAuth() @UseGuards(JwtAuthGuard, RolesGuard) @Roles(UserRole.ADMIN) @Controller('admin/banners')
...
@ApiTags('Catalog - Banners') @Controller('banners')
export class CatalogBannersController {
  constructor(private readonly service: BannersService) {}
  @Get() findAll() { return this.service.findAll(true); }
}
