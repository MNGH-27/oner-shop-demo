...
const DEMO_IMAGES: Record<string, string> = {
  'oner-product-1.jpg': 'https://www.asterandoak.com.au/cdn/shop/files/0G7A7231.jpg?v=1759911866',
  'oner-product-2.jpg': 'https://bowymade.com/cdn/shop/files/muslin-bubble-blanket-cream-baby-12_1800x1800.jpg?v=1770693481',
  'oner-product-3.jpg': 'https://www.woollykins.com.au/cdn/shop/files/Fibre-For-Good-Organic-Coloured-Cotton-Muslin-Baby-Blanket-Throw-04_5000x.jpg?v=1732605602',
  'oner-product-4.jpg': 'https://moonjax.com/cdn/shop/products/File_000.jpg?v=1660060991&width=2000',
...
export class SeedService implements OnModuleInit {
  ...
  private async refreshExistingDemoImages() {
    const activeProducts = await this.productModel.find({ isActive: true }).sort({ createdAt: 1 }).limit(4).exec();
    for (let index = 0; index < activeProducts.length; index += 1) {
      const image = this.uploadPath(`oner-product-${index + 1}.jpg`);
      activeProducts[index].images = Array.from(new Set([...(activeProducts[index].images ?? []), image]));
      await activeProducts[index].save();
    }
